...
They are NOT at all designed to power each other, as each has an individual Li-Po battery connected to it via 2 extra pins at the top of the board (called B+ and B-). Using other means (TRRS) to share power is problematic and should be avoided (see below).
...
This is highly dependent on the battery size and features of the keyboard. The [power profiler](https://zmk.dev/power-profiler) from ZMK offers a good estimate of the battery life you can expect.
...
No, there's a connection via BLE between the two boards. The central half reports back the keystrokes of both the central (usually left) and peripheral (usually right) sides. Infact using a TRRS jack with the batteries still connected is strongly discouraged, as this (because of the inherent design of the TRRS) can cause a short ciruit 
and damage the IO pins on the nice!nano.

### QMK firmware support?

This is complicated. Nordic's nRF52 line has some licensing issues with its SDK making it not possible to be upstreamed to the main QMK repo. Instead, we rely on other firmwares like [ZMK or BlueMicro](/docs/wireless-firmware/) that offer a great deal of functionality with full legality and wireless focus.
