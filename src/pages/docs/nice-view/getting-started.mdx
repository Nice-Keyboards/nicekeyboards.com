...
### Using an existing OLED compatible shield
 ...
 - Create a bodge wire from the CS pin to the D1 Pro Micro pin (P0.06/006 on the nice!nano)
...
After successfully installing your nice!view, all that should be required is to build your board with the `nice_view` shield.

### ZMK Config Repo

With a ZMK config repo, edit the `build.yaml` file to have ` nice_view` added to the end of each shield string.
If you're using an OLED compatible shield, you'll also need to add `nice_view_oled` to the list of shields *first*.
...
+   shield: lily58_left nice_view_oled nice_view
  - board: nice_nano_v2
-   shield: lily58_right
+   shield: lily58_right nice_view_oled nice_view
```
...
+ west build -p -b nice_nano_v2 -- -DSHIELD="lily58_left nice_view_oled nice_view"
```
